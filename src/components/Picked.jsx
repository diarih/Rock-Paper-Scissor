    ...
    <div className={'flex gap-14 p-4'}>
      ...
      </div>
      {
        house && (
          <div className={'flex flex-col gap-4 items-center justify-center transition'}>
            ...
            <button
              onClick={() => {onReset()}}
              className={'rounded p-8 w-full py-4 bg-white hover:text-red-500 text-gray-700 transition uppercase font-semibold text-xs'}>play
              again
            </button>
          </div>
        )
      ...
      <div>
        ...
        <div className={'flex items-center justify-center'}>
          {
            loading ? (
              <div className="animate-ping mt-14 md:h-20 md:w-20 h-16 w-16 rounded-full bg-sky-400 opacity-75">
              </div>) : (
